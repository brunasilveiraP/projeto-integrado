...
namespace BBL.Users
{
    ...
    public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UserDto>, IUserAppService
    {
        ...
        public override async Task<UserDto> CreateAsync(CreateUserDto input)
        {
            ...
            var t = await _userManager.CreateAsync(user, "d84#Z3");

            CheckErrors(t);

            if (input.RoleNames != null)
            {
                if (input.RoleNames[0] == null)
                {
                    input.RoleNames[0] = "Parceiro";
                }
            ...
            await _identidadeService.SendNewUserPasswordAsync(user);
        ...
        protected override IQueryable<User> CreateFilteredQuery(PagedUserResultRequestDto input)
        {
            ...
            return Repository.GetAll()
                .Include(x => x.Roles)
                .Include(x => x.Tenant)
                .WhereIf(AbpSession.TenantId != null, x => x.TenantId == AbpSession.TenantId)
                .WhereIf(!input.SearchText.IsNullOrWhiteSpace(),
                    x => x.UserName.Contains(input.SearchText) ||
                         x.Name.Contains(input.SearchText) ||
                         x.EmailAddress.Contains(input.SearchText) ||
                         x.Tenant.RazaoSocial.Contains(input.SearchText) || x.Tenant.Fantasia.Contains(input.SearchText)
                         );
        }
        ...
        public IEnumerable<UserSimpleDto> GetAllUsers()
        {
            ...
            return ObjectMapper.Map<IEnumerable<UserSimpleDto>>(users);
        }
        ...
        public override async Task<PagedResultDto<UserDto>> GetAllAsync(PagedUserResultRequestDto input)
        {
            try
            {
                using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MustHaveTenant, AbpDataFilters.MayHaveTenant))
                {
                    var t = await base.GetAllAsync(input);
                    return t;
                }
            }
            catch (Exception e)
