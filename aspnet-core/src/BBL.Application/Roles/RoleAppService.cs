...
namespace BBL.Roles
{
    ...
    public class RoleAppService : AsyncCrudAppService<Role, RoleDto, int, PagedRoleResultRequestDto, CreateRoleDto, RoleDto>, IRoleAppService
    {
        ...
        public RoleAppService(IRepository<Role> repository, RoleManager roleManager, PermissionManager permissionManager, UserManager userManager, IUnitOfWorkManager unitOfWorkManager)
            : base(repository)
        {
        ...
        public async Task UpdateRolePermissions()
        {
            try
            {
                           ...
                           AbpDataFilters.MayHaveTenant))
                {
                    ...
                    IEnumerable<string> permissionsT = new List<string>(){"Pages.Visualizacao"};
                    var roles = Repository.GetAll().ToList();
                    var role = await _roleManager.GetRoleByIdAsync(2);
                    var grantedPermissions = _permissionManager
                        .GetAllPermissions()
                        .Where(p => permissionsT.Contains(p.Name))
                        .ToList();
